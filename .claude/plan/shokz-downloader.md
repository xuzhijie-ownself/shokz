...
| **8a** | Safety primitives (dormant library code) | *FileLockPolicy + DiskGuardPolicy + 4 new domain errors land as reviewed primitives, ready to be wired in 8b -- no behavior change to `shokz download`* | `v0.9.0` | `domain/errors.py` (+ AnotherRunInProgress, StaleLock, LockOwnerUnknown, DiskFull with structured attrs), `config/schema.py` ([disk] + [lock] sections), `policies/file_lock.py` (5-step classification, sibling meta atomic write), `policies/disk_guard.py` (batch-level pre-flight, humanfriendly binary). 22 unit tests across 3 files. Split mid-Phase-3 per GAN Option C verdict. | ~½ day delivered |
| **8b** (v1.0 ⭐) | Wire safety primitives + SIGINT + 3 ENOSPC sites | *CLI acquires lock pre-asyncio.run; SIGINT cleanly cancels with subprocess SIGTERM-then-wait-then-unlink; disk guard pre-flight runs once after resolve-all; ENOSPC translates at ffmpeg-stderr / os.replace / manifest append* | `v1.0.0` | Wiring of v0.9.0 primitives into use case + CLI; asyncio.shield+drain manifest.record; finally-block raw cleanup; SIGINT integration test; 3 ENOSPC translation sites + tests. | ~½ day |
| **8.5** | `shokz retry` command | *Re-run failed tracks from `failures.jsonl` without manual URL list assembly* | `v1.0.1` | `shokz retry` reads `failures.jsonl`, dedupes by track_id, invokes the download use case with the failed URLs. Deferred from Sprint 8 per its GAN review (M4): keeping v1.0 focused on the safety primitives. | ~½ day |
