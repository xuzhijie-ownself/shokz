...
## Sprint 9 -- Symlink rejection lifted to CLI layer (M1 carry-forward) -- 2026-04-30
**Goal:**         Lift symlink rejection from `BatchDownloadUseCase` into a shared `_runtime` helper so `shokz download / playlist / retry` all reject symlinked `--output` BEFORE acquiring the lock. Tag v1.0.2.
**Shipped?:**     yes (v1.0.2)
**Time actual:**  ~30 minutes across RED1/GREEN1/RED2/GREEN2/commit. The smallest sprint since Sprint 6's "drop concurrency, change one default" patch.
Keep:             Strict TDD discipline at micro-scale. RED1 (helper unit tests) failed with ImportError -- a satisfyingly explicit RED. GREEN1 made 4/4 pass with a ~25-line helper. RED2 (CLI integration tests via Typer CliRunner) failed exactly 3/3 because the helper wasn't wired -- another clean RED. GREEN2 added 6 lines x 3 commands. Total 273 tests passing. The cycle is faster at this scale than spec-write-spec-GAN-spec-amend-implement-GAN, and STILL produces the same provable behavior. Use TDD-first when the change is small enough to specify in test docstrings (Gherkin-equivalent); save the spec-doc treatment for half-day-plus sprints.
Drop:             Spent ~5 minutes considering whether to extract the 3-command `try/except NameOutsideOutputDir/sys.exit(1)` pattern into a runtime helper. Architect's "wait for 4th command" verdict from Phase C still applies -- 3 x 5 lines of explicit try/except is more grep-friendly than a wrapper. Refactor only when duplication crosses 4-5 instances.
Try next:         Two paths, in priority order: (a) `shokz doctor` diagnostic command (ffmpeg/yt-dlp present, [disk] free, output_dir writable). README forward-list entry from Sprint 0. ~1/2-1 day. (b) Push to GitHub remote so future Sprint X work can be scheduled via remote agents (currently all 13 tags are local-only). 5 minutes.
Surprise:         The integration tests that assert "no `.shokz/locks/` dir was created" turned out to be the highest-leverage assertion. The original M1 finding was a UX issue ("waste of lock acquire on symlinked target"), but locking the test as "lock dir doesn't exist post-rejection" makes the contract concrete: the symlink check fires BEFORE FileLockPolicy's lazy mkdir at __enter__. A regression that re-introduces the late check would still raise NameOutsideOutputDir (so the exit-code test would pass), but the lock dir would exist on the symlink target -- which IS the silent-failure mode the original M1 finding called out. Test-the-side-effect-not-the-error pattern earned its slot.


## Sprint 10 -- shokz doctor read-only diagnostics -- 2026-04-30
**Goal:**         New `shokz doctor` command surfaces ffmpeg / ffprobe / yt-dlp / output_dir / disk-free issues BEFORE the user hits a runtime failure. Read-only. Tag v1.1.0 (minor bump per master plan).
**Shipped?:**     yes (v1.1.0)
**Time actual:**  ~45 minutes across drift-fix + RED1/GREEN1/RED2/GREEN2/commit. Roughly 2x Sprint 9's micro-cycle, matching the larger surface (new use case + new CLI command + dataclasses + 9 tests).
Keep:             Callable-based dependency injection (which / disk_free_bytes / ytdlp_version) on the use case constructor instead of inventing a `DiagnosticPort` Protocol. Hexagonal-lite: keeps the use case pure-function-y and unit tests drive every branch without inventing a port that has exactly one impl. Also: pre-flight drift detection caught a real user-facing stale claim in README.md ("single-process-safe only until Sprint 8") that lived through 4 prior tags untouched -- doing a grep pass for stale Sprint references SHOULD be standard at the start of every sprint, not occasional.
Drop:             Initially considered putting CLI integration tests for doctor in `test_cli_smoke.py` (mirror of where download/playlist live). Switched mid-flight to keeping them in `test_doctor.py` so the use case + CLI live together -- one file = one feature surface = one place to grep when reading. Lesson: organise test files by feature seam, not by transport layer (smoke / runner / etc.). The cli/runtime helper tests in test_cli_runtime.py followed this pattern naturally; should have applied it from the start.
Try next:         Two paths still pending: (a) push to GitHub remote so future remote-agent automation works (5 minutes; all 14 tags currently local-only). (b) Phase D-style polish that the master plan §"v1.1.0" originally bundled with doctor -- "library verify + JSON observability". Honest call: those are separate ½-day sprints each; the master plan v1.1.0 = doctor-only is the right scoping with hindsight.
Surprise:         Live `shokz doctor` against `--output /tmp/shokz-doctor-eval` correctly reported FAIL on the `output_dir` check because /tmp on macOS is a symlink to /private/tmp. That's not a test-fixture pathology -- it's the real macOS-Sequoia filesystem layout, and the diagnostic just caught it on its first live invocation. The test that verified "ancestor symlinks raise" turned out to be EXACTLY the production-realism contract the user needs. Strict TDD's RED-first discipline is also a "did I think about this case?" forcing function: the ancestor-symlink test was added in Sprint 9 explicitly as defense-in-depth, and Sprint 10's first real run vindicated the call.


## Sprint 11 -- shokz split (hour-sized MP3 parts) -- 2026-04-30
**Goal:**         Chop long MP3s into hour-sized parts so an 11-hour audiobook becomes 12 navigable tracks on a device with no usable underwater seek UI. Lossless. Tag v1.2.0.
**Shipped?:**     yes (v1.2.0)
**Time actual:**  ~40 minutes across RED1/GREEN1/RED2/GREEN2/live-run/docs. Comparable to Sprint 10 despite touching a Port (which Sprint 10 did not).
Keep:            THE FEATURE CAME FROM ACTUALLY USING THE TOOL. Sprint 10's retro said "stop building, go use it" -- and the very first real download (an 11.4-hour Chinese audiobook, 312 MB, one file) produced a concrete, unarguable feature request within minutes. Every sprint from 0-10 was planned; Sprint 11 was *discovered*. That is a materially better way to pick work, and the honest lesson is that the "use it for a week" advice should have come several sprints earlier. The counterfactual: had I speculatively built `--split-hours` into `download` back at Sprint 5, I'd have coupled it to the manifest and paid a schema migration for a feature whose real shape (post-processing on any file, not a download mode) I could not have guessed.
Drop:            Scoping this as a `download --split-hours N` flag (my Option 3, offered and rejected). It sounds like the "integrated" answer but it forces 1-URL-to-N-manifest-rows, which breaks skip-existing's `(source, track_id)` keying and needs a schema_version=2 migration. Standalone `shokz split` operating on any file on disk sidesteps that entirely and is *more* useful (works on files shokz didn't download). Lesson: when a feature request seems to demand a schema change, first check whether a decoupled tool gets 100% of the value at 10% of the cost. It usually does.
Try next:        Live-use again before building anything. Two known-real items if they resurface: (a) `shokz retry` and `shokz library` silently default to `./downloads/` relative to cwd, so they miss a library created with `download --output ~/Music/shokz` -- a genuine ergonomic bug I hit *during this session* and worked around with an env var. That is the next real sprint if it annoys me twice. (b) 52-hour videos (2.5 GB) fail on YouTube's CDN with HTTP 500; shokz classifies and logs it correctly, but there's no resume, so each retry restarts from zero.
Surprise:        [see Sprint 12 -- this entry's optimism did not survive contact with an audit.] The `-c copy` stream-copy did the 312 MB / 11.35-hour split in **4.25 seconds** -- and ffprobe confirmed the 12 parts sum to exactly 11.35 hours at the original 64 kbps, so not a single sample was lost or re-encoded. I had budgeted "seconds not minutes" in the port docstring as an aspiration; it beat that by an order of magnitude. Worth internalising: for any "chop / concat / remux" audio task, `-c copy` is nearly free, and reaching for a re-encode (which here would have cost ~5 minutes and a generation of quality) is almost always the wrong instinct.


## Sprint 12 -- Split, honestly (fixing the bug Sprint 11 shipped) -- 2026-07-14
**Goal:**        Kill the HIGH-severity silent corruption `shokz split --force` shipped in v1.2.0, at the root rather than the symptom. Tag v1.2.1.
**Shipped?:**    yes (v1.2.1)
**Time actual:** ~3 hours: an evidence-gathering audit, then 4 TDD waves, then an adversarial review of my own fix, then 2 review-driven hardening fixes.
Keep:            **The audit that found it.** Asked "what is next?" and, instead of opining, ran a fan-out audit (4 finder lenses -> independent skeptics tasked with REFUTING each finding -> an architect synthesising what survived). It found that the feature I had shipped ONE SPRINT EARLIER silently corrupted its own advertised workflow. I then reproduced both bugs by hand against real ffmpeg before believing the reviewers -- and the mtimes told the whole story: 4 fresh parts at 21:39:21, 7 stale ones at 21:39:19, all 11 reported as freshly written. Do not take a reviewer's word for a HIGH; go and make it fail.
Drop:            **Trusting a green suite over a real binary.** Sprint 11 had 14 passing tests and shipped corruption, for one reason: *the fake was better-behaved than production.* The fake returned what it WROTE; the adapter returned what EXISTED (walk 1..N until a gap). Those agree only when the output dir starts empty -- i.e. in every test, and in none of the interesting real cases. `FfmpegEncoder.segment()` had ZERO direct tests. The rule I am writing down: **when a fake is simpler than the thing it stands in for, that difference IS the bug you are about to ship.** The fake is now bound to the same `part_name()` the adapter uses, and the adapter has 9 tests against real ffmpeg, deliberately NOT gated behind INTEGRATION=1 -- gating them is how this shipped.
Try next:        v1.2.2 follow-ups, none urgent: (a) '%' in the output DIRECTORY still reaches ffmpeg's printf expander (pre-existing; v1.2.0 crashed outright, so this is strictly better -- fix is `cwd=scratch` + relative template); (b) `stderr.splitlines()[-1:]` throws away ffmpeg's diagnostic line, making field failures undiagnosable; (c) reconciliation should exclude `.shokz-*` by prefix, not exact name. And the Sprint 11 carry-forward still stands: `library` has no `--output` and silently reports "(no entries)" against the wrong directory.
Surprise:        **The review of my fix was as valuable as the review that found the bug -- and its judge was more valuable than either.** 26 claims, 18 survived refutation, and the judge threw out most of the severity by doing the one thing no attacker did: *diffing against the shipped baseline instead of an imagined safe one*. Four lenses graded "--force deletes before segmenting" as a HIGH new regression. But v1.2.0's cleanup ALREADY destroyed the old split on ffmpeg failure (that was bug B2), and was WORSE on Ctrl+C. Exactly one branch was genuinely new (ffmpeg missing from PATH), and the "obvious" fix three lenses prescribed -- segment first, prune after -- would have TRIPLED peak disk, making the DiskFull failure they cited as motivation strictly *more* likely. Two lessons, and the second is the sharper one: an adversarial reviewer with no baseline will reliably invent regressions; and **blocking a corruption fix to polish it has a cost too -- the corruption stays live.** I still took the two cheap findings (a swallowed `unlink` OSError that re-opened the bug's on-disk state, and an ffmpeg pre-flight before the destructive delete) because shipping the same corruption twice in two days would have been unforgivable.
